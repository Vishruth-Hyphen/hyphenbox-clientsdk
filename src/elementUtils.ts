export class ElementUtils {
    ...
    // New method to find elements with fuzzy matching
    static findElementWithFuzzyLogic(interaction: any): HTMLElement | null {
      const element = interaction.element || {};
      
      console.log('[FUZZY-FINDER] Starting search with fuzzy matching for:', {
      ...
      const allCandidates = Array.from(document.querySelectorAll(element.tagName || '*'));
      console.log(`[FUZZY-FINDER] Found ${allCandidates.length} potential candidates`);
      ...
      const scoredCandidates = allCandidates.map(candidate => {
        ...
        if (element.textContent) {
          // Check for exact text match (highest score)
          if (candidateEl.textContent?.trim() === element.textContent) {
            score += weights.exactTextMatch;
          } 
          // Check for contained text match (lower score)
          else if (candidateEl.textContent?.includes(element.textContent)) {
            score += weights.containsText;
          }
        ...
        // TAG NAME MATCHING
        if (element.tagName && candidateEl.tagName === element.tagName) {
          score += weights.tagNameMatch;
        }
        
        // ID PATTERN MATCHING - don't require exact ID, just pattern
        if (element.id && candidateEl.id) {
          // Check if IDs share a pattern (like same suffix after a random part)
          const elementIdParts = element.id.split('-');
          const candidateIdParts = candidateEl.id.split('-');
          ...
          if (elementIdParts.length === candidateIdParts.length) {
            ...
            // Award points based on how many parts match
            score += (matchingParts / elementIdParts.length) * weights.idPatternMatch;
          }
        }
        
        // CLASS MATCHING - check if classes contain similar patterns
        if (element.className && candidateEl.className) {
          ...
          const commonPatterns = elementClasses.filter((cls: string) => {
            return candidateClasses.some((candCls: string) => {
              ...
              if (clsParts.length !== candClsParts.length) return false;
              
              // Check if at least some parts match (first and last are most important)
              return (clsParts[0] === candClsParts[0] || 
                      clsParts[clsParts.length-1] === candClsParts[candClsParts.length-1]);
            });
          }).length;
          
          score += (commonPatterns / elementClasses.length) * weights.classPatternMatch;
        }
        
        // ROLE ATTRIBUTE MATCHING - important for accessibility and framework components
        if (element.attributes?.role && candidateEl.getAttribute('role') === element.attributes.role) {
          score += weights.roleMatch;
        }
        
        // POSITION MATCHING - elements in similar position on page
        if (interaction.elementRect && candidateEl.getBoundingClientRect) {
          const candidateRect = candidateEl.getBoundingClientRect();
          const originalRect = interaction.elementRect;
          
          // Calculate position similarity based on distance
          const xDistance = Math.abs((originalRect.left + originalRect.width/2) - 
                                   (candidateRect.left + candidateRect.width/2));
          const yDistance = Math.abs((originalRect.top + originalRect.height/2) - 
                                   (candidateRect.top + candidateRect.height/2));
          
          // Closer elements get higher scores
          const positionScore = Math.max(0, weights.positionMatch - 
                                      (xDistance + yDistance) / 10);
          score += positionScore;
        }
        
        // VISIBILITY - prefer visible elements
        if (candidateEl.offsetParent !== null) {
          score += weights.visibilityMatch;
        }
      ...
      console.log('[FUZZY-FINDER] Top 5 candidates:');
      for (let i = 0; i < Math.min(5, scoredCandidates.length); i++) {
        const candidate = scoredCandidates[i];
        console.log(`Candidate #${i+1} (score: ${candidate.score}):`, {
          element: candidate.element,
          id: candidate.element.id,
          text: candidate.element.textContent?.substring(0, 50),
          tagName: candidate.element.tagName,
          classes: candidate.element.className
        });
      }
      
      // Return the highest scoring candidate if score is above threshold
      if (scoredCandidates.length > 0 && scoredCandidates[0].score > 50) {
        console.log(`[FUZZY-FINDER] Found best match with score ${scoredCandidates[0].score}`);
        return scoredCandidates[0].element;
      }
      
      console.log('[FUZZY-FINDER] No suitable match found');
      return null;
    }
}